...
> ⚠️ **未绑定的群**：消息不会转发进游戏，AI 也拿不到「谁是不是管理员」的身份。
> 但**执行指令与查好感的工具仍然可用**（工具是按插件注册的，不按群区分）——
> 只是那边 AI 不知道对方身份。若你不想让未绑定群碰服务器，请把机器人从那些群
> 移出，或用 AstrBot 的 `plugin_set` 限制插件生效范围。
...
- **管理员待遇也由提示词决定**：插件只把「谁是管理员」作为事实告诉 AI（写在 `<netherlink_context>` 里），并不规定该怎么对待他。给管理员更宽松的尺度是在 `karma_rules` 里写的——想收紧或放宽，改那段提示词即可。
  这段文本本身可在 `template_netherlink_context` 里配置（改措辞、换标签名、增删句子）；游戏内对话另有一段附加说明，在 `template_netherlink_request` 里配置。
